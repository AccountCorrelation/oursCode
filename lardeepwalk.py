...
def start(father):
    if comm_rank==0:
        ...
        ###modelX=Word2Vec(walkListX,negative=10,sg=1,hs=0,size=800,window=4,min_count=0,workers=15,iter=30)

        #del(walkListX)
        
        ###walkListY = word2vec.LineSentence(father+'walk1all_Y.txt')#'walkListY.txt')
        ###modelY=Word2Vec(walkListY,negative=10,sg=1,hs=0,size=800,window=4,min_count=0,workers=15,iter=30)


        gc.collect()

        ###modelX.init_sims(replace=True);modelY.init_sims(replace=True)
        ###modelX.save_word2vec_format(father+'lx1_tang15_undirect_iter30_wind4_800s.emb', binary=False, fvocab=father+'modelx.vocab')
        ###modelY.save_word2vec_format(father+'ly1_tang15_undirect_iter30_wind4_800s.emb', binary=False, fvocab=father+'modely.vocab')
        print "save ok !";
        gc.collect()
        ...
        fr = open(father+'trainConnect3148.txt')
        ...
        ft=open(father+'trainConnect3148.txt','r')
        ...
        ft=open(father+'testConnect3148.txt','r')
        for line in ft.readlines():
	    lineArr = line.strip().split()
	    nodeX=lineArr[0]
	    nodeY=lineArr[1]
